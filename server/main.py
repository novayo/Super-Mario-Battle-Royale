...
async def handle_message(payload, send_func):
    ...
    try:
        ...
        with state_lock:
            # Get or create player_id for this connection
            if send_func not in CONNECTION_TO_PLAYER:
                global NEXT_PLAYER_ID
                player_id = f"p{NEXT_PLAYER_ID}"
                NEXT_PLAYER_ID += 1
                CONNECTION_TO_PLAYER[send_func] = player_id
            else:
                player_id = CONNECTION_TO_PLAYER[send_func]

            # Create a PlayerUpdate for server-to-client broadcast
            server_update = player_pb2.PlayerUpdate(
                player_id=player_id,
                x=update.x,
                y=update.y,
            ...
            PLAYERS[player_id] = server_update
        ...
        await broadcast(game_state.SerializeToString(), exclude=send_func)

    except Exception as e:
        logger.error(f"Protobuf communication error: {e}")


async def broadcast(data, allow_rebroadcast=True, exclude=None):
    """Sends binary data to all active connections."""
    with state_lock:
        connections = list(CONNECTIONS)
    for send in connections:
        if exclude and send == exclude:
            continue
        # Check if async
